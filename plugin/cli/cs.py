...
# execve). Resolution (see adr/0001-cli-version-dispatch.md): setup / status /
# install-cli run the NEWEST installed version (they must work before a project
# is pinned); every other command runs the project's pinned version VERBATIM —
# no auto-selection, no version drift. An unpinned project, or a pin not in the
# store, errors and tells the user to run setup. Rewritten verbatim on every
...
edit. Runs the project's pinned CLI version verbatim; setup/status/install-cli run
the newest installed version. See adr/0001-cli-version-dispatch.md."""
...
# Commands that must run without a project pin: setup/install-cli (maintenance)
# and status (pre-setup diagnostic). Everything else runs the project's pin.
_NEWEST_CMDS = ("setup", "install-cli", "status")
...
def main():
    ...
    if _subcommand(argv) in _NEWEST_CMDS:
        target = entries[max(entries, key=_semver)]
    else:
        ...
        if not want:
            ...
            sys.exit(1)
        target = entries.get(want)
        if target is None:
            print(f"unity-cli: this project is pinned to CLI {want}, which is not installed in the "
                  f"store. Run the unity-cli-setup skill to install it.", file=sys.stderr)
            sys.exit(1)
